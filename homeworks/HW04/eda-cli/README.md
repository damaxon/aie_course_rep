...
`/quality-flags-from-csv` и `/head-from-csv`.  
...
- в зависимости добавлены `fastapi`, `uvicorn[standard]` и `typing`.
...
### 6. `POST /head-from-csv` – первые n-строк из CSV-файла

Эндпоинт принимает CSV-файл, внутри:

- читает его в `pandas.DataFrame`;
- возвращает n-строки датасета и базовую информацию (n_rows,n_cols).
...
```bash
curl -X POST "http://127.0.0.1:8000/head-from-csv" \
  -F "file=@data/example.csv" \
  -F "n=6"
```
...
- базовую информацию о датасете
- `data` - первые n-строк CSV файла.
