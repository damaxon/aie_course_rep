...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    user_id_col_sum = [colsum for colsum in summary.columns if colsum.name=="user_id"]
    if user_id_col_sum:
        flags["has_suspicious_id_duplicates"] = not(summary.n_rows == user_id_col_sum[0].unique)
    else: flags["has_suspicious_id_duplicates"] = "Error: columns with tag \"user_id\" does not exist"

    flags["has_constant_columns"] = any([True for colsum in summary.columns if colsum.unique==1])

    flags["has_high_cardinality_categoricals"] = any([True for colsum in summary.columns if (colsum.unique>30 and colsum.name!="user_id")])
