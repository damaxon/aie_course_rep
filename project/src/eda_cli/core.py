...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    """
    flags: Dict[str, Any] = {}
    flags["too_few_rows"] = summary.n_rows < 1000
    flags["too_many_columns"] = summary.n_cols > 10

    max_missing_share = float(missing_df["missing_share"].max()) if not missing_df.empty else 0.0
    flags["max_missing_share"] = max_missing_share
    flags["too_many_missing"] = max_missing_share > 0.5

    filename_col_sum = [colsum for colsum in summary.columns if colsum.name=="filename"]
    if filename_col_sum:
        flags["has_suspicious_filename_duplicates"] = not(summary.n_rows == filename_col_sum[0].unique)
    else: flags["has_suspicious_filename_duplicates"] = "Error: columns with tag \"filename\" does not exist"

    flags["has_constant_columns"] = any([True for colsum in summary.columns if colsum.unique==1])

    flags["has_invalid_coord_bounding_box"] = any([True for colsum in summary.columns if (colsum.name in ["xmin","xmax","ymin","ymax"] and (colsum.min<0 or colsum.max>512))])
    
    flags["has_invalid_image_resolution"] = any([True for colsum in summary.columns if (colsum.name in ["width","height"] and (colsum.min==colsum.max!=512))])
    ...
    if summary.n_cols > 100:
        score -= 0.1
    if flags["has_suspicious_filename_duplicates"]==True:
        score -= 0.15
    if flags["has_constant_columns"]==True:
        score -=0.05
    if flags["has_invalid_image_resolution"]==True:
        score -=0.15
    if flags["has_invalid_coord_bounding_box"]==True:
        score -=0.05
